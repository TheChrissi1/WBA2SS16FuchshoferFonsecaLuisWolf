...
router.put('/user', jsonParser, function(req, res){
    ...
    //User ID und Authority müssen unabhängig vom req.body verändert werden.
    var User = req.body;
    var newUser = {
        user_id:0,
        'name':User.name,
    ...
    db.keys('user:*' , function(err, rep){
        if(err) throw err;
        var result = rep;
        //Wenn noch kein User in Redis vorhanden ist:
        if(result.length > 0){
            db.mget(result, function(err, rep){
                ...
                db.incr('user_id:user', function (err, rep) {
                    ...
                    //Neuer User wird in Redis eingetragen.
                    ...
                    //Gewählter username ist schon vorhanden.
                } else {
            ...
            //Noch kein User in Redis vorhanden:
            } else {
                db.incr('user_id:user', function (err, rep) {
                    if(err) throw err;
...
router.put( '/user/:user_id', jsonParser, function(req, res){
    ...
    db.exists('user:'+req.params.user_id, function(err, rep) {
		...
		if (rep == 1) {
            ...
            //Der req.body wird als Datensatz für den User gespeichert.
        ...
        //User existiert nicht.
			...
			res.status(404).type('text').send('User does not exists!');
...
router.put( '/user/:user_id/stats', jsonParser, function(req, res){
    ...
    db.exists('stats:'+req.params.user_id, function(err, rep){
        ...
        if(rep == 1){
            //Statistik wird anhand es req.body aktuallisiert
        ...
        //Falls Usernicht vohanden ist existiert auch keine Statistik.
        } else {
...
//[OK]
//Registriert einen neuen Benutzer mit Passwort.
router.put('/signup', jsonParser, function(req, res){
    ...
    db.get('auth:' + user.username, function(err, rep){
        if(err) throw err;
        if( !rep ){
            //Dem username wird ein passwort zugewiesen.
...
router.put('/login', jsonParser, function(req, res){
    ...
    db.get('auth:' + User.username, function( err, rep ){
        ...
        if( rep ){
            ...
            //Useraccount ist aktuell deaktiviert.    
            } else {
                res.sendStatus(423);
