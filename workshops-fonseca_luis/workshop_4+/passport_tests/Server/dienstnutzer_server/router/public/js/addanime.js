...
function genreToggle(){
  ...
  if(bool == true){
    for(var i = 0; i<tmp.length; i++){
      ...
      tmp[i].style.display="";
      document.getElementById('genre-button').innerHTML = "Hide";
    }
    bool = false;
  } else {
    for(var i = 0; i<tmp.length; i++){
      ...
      document.getElementById('genre-button').innerHTML = "Show";
    }
    bool = true;
  }
}
...
function refToggle(){
  var tmp = document.getElementsByClassName('refs-param')
  genreNum = tmp.length;
  if(bool == true){
    for(var i = 0; i<tmp.length; i++){
      tmp[i].style.visibility="visible"
      tmp[i].style.display="";
      document.getElementById('refs-button').innerHTML = "Hide";
    }
    bool = false;
  } else {
    for(var i = 0; i<tmp.length; i++){
      tmp[i].style.visibility="hidden"
      tmp[i].style.display="none";
      document.getElementById('refs-button').innerHTML = "Show";
    }
    bool = true;
  }
}
